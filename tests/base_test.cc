...
TEST(MetadataTest_St209450Roundtrip, St209450Roundtrip) {
  ...
  ASSERT_OK_AND_ASSIGN(std::string st209450_data, ToSt209450(metadata));

  ASSERT_OK_AND_ASSIGN(DynamicMetadata metadata_rt,
                       FromSt209450(st209450_data));
...
TEST(MetadataTest_St209450Roundtrip, InvalidMetadataWrongSign) {
  ...
  EXPECT_THAT(ToSt209450(metadata),
              StatusIs(absl::StatusCode::kInvalidArgument,
                       HasSubstr("Sign of y does not match")));
}
...
TEST(MetadataTest, FailsOnCurveSize) {
  ...
  EXPECT_THAT(ToSt209450(metadata),
              StatusIs(absl::StatusCode::kInvalidArgument,
                       HasSubstr("curve size 33 is > 32")));

  metadata.rules[0].curve.clear();
  EXPECT_THAT(ToSt209450(metadata),
              StatusIs(absl::StatusCode::kInvalidArgument,
                       HasSubstr("curve size 0 is > 32 or == 0")));
}
...
TEST(MetadataTest, FromSt209450RwtmFlag) {
  ...
  ASSERT_OK_AND_ASSIGN(DynamicMetadata metadata, FromSt209450(data));
